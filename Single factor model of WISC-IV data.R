...
# Single factor model of WISC-IV data
library(lavaan)
library(semPlot)
wisc4.cor.lwr <- c(1,
                   0.72,1,
...
wisc4.model<-'
g =~ a*Information + b*Similarities + 
     c*Word.Reasoning + d*Matrix.Reasoning + e*Picture.Concepts'
wisc4.fit <- lavaan::cfa(model=wisc4.model, 
                         sample.cov=wisc4.cov, 
                         sample.nobs=550,
                         std.lv=TRUE)
lavaan::varTable(wisc4.fit)
lavaan::summary(wisc4.fit, fit=TRUE, std=TRUE, rsq=TRUE)
semPlot::semPaths(wisc4.fit,
                  layout="tree",
                  style="lisrel",
                  what="path",
                  residuals=TRUE,
